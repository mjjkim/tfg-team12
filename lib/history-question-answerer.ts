...
export function answerHistoryQuestion(query: string, analysis: TimeSeriesAnalysis): string {
  ...
  if (q.includes('가장 많이 오른') || q.includes('최고 상승') || q.includes('최대 상승')) {
    ...
    return `가장 많이 오른 구간은 ${candidate.startDate}부터 ${candidate.endDate}까지 약 ${candidate.changePct.toFixed(1)}퍼센트 상승한 구간입니다.`;
  }
  ...
  if (q.includes('가장 많이 내린') || q.includes('최대 하락') || q.includes('가장 많이 떨어진')) {
    ...
    return `가장 많이 내린 구간은 ${candidate.startDate}부터 ${candidate.endDate}까지 약 ${Math.abs(candidate.changePct).toFixed(1)}퍼센트 하락한 구간입니다.`;
  }

  if (q.includes('최고가')) {
    return `최고가는 ${analysis.summary.highestDate} 기준 ${analysis.summary.highestPrice.toLocaleString('ko-KR')}원입니다.`;
  }
  if (q.includes('최저가')) {
    return `최저가는 ${analysis.summary.lowestDate} 기준 ${analysis.summary.lowestPrice.toLocaleString('ko-KR')}원입니다.`;
  }
  ...
  if (q.includes('횡보')) {
    ...
    return `횡보 구간은 ${side.startDate}부터 ${side.endDate}입니다.`;
  }
  if (q.includes('변동성')) {
    const target = [...analysis.segments].sort((a, b) => b.volatilityPct - a.volatilityPct)[0];
    if (!target) return '변동성 분석 결과가 부족합니다.';
    return `변동성이 가장 큰 구간은 ${target.startDate}부터 ${target.endDate}로 약 ${target.volatilityPct.toFixed(1)}퍼센트입니다.`;
  }
