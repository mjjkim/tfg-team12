...
export default function FallbackControls({ title, onSubmit, examples }: FallbackControlsProps) {
  ...
  return (
    <section className="card" aria-live="polite">
      <h2 className="text-lg font-bold">음성 입력 대체</h2>
      ...
      <form className="flex flex-col gap-2" onSubmit={onFormSubmit}>
        ...
        <input
          ...
          placeholder="명령어를 입력해 주세요"
          aria-label="텍스트 입력으로 음성 명령 대체"
        />
        <button type="submit" className="rounded-lg bg-cyan-400 text-slate-950 px-4 py-3 min-h-[48px] font-bold">
          실행
        </button>
      </form>
